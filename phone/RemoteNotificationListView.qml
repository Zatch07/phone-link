 ...
 * `(ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).groupsByAppName` / `appNameList`. Each group
 ...
 *
 * Source of truth: `(ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).notifications` (only the active
...
StyledListView {
    ...
    property string deviceId: (ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).activeDeviceId || ""
    ...
    model: ScriptModel {
        values: (ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).appNameList
    }
    delegate: RemoteNotificationGroup {
        ...
        notificationGroup: {
            const g = (ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).groupsByAppName[modelData]
    ...
    PagePlaceholder {
        anchors.fill: parent
        shown: (ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).notifications.length === 0
        icon: "notifications_off"
        description: {
            if ((ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).activeReachable
                    && (ExtensionServices.loaded["phone-link.KdeConnectService"] || {}).activeDevice
